...
/**
 * 
 * @brief Applies EI "fix short axis" crop+resize to a raw grayscale camera frame.
 *
...
/**
 * @brief An abstraction layer function that acts as the data retrieval interface 
 *        for the Edge Impulse signal wrapper.
 *        Maps a localized request from the inference engine to the preprocessed, 
 *        statically indexed camera frame buffer, iteratively normalizing each 
 *        pixel from an 8-bit integer channel into a floating-point representation.
 *        Normalization: uint8 [0,255] → float [0.0, 1.0]
 * @param offset The starting position within the target feature stream.
 * @param length The total number of signal elements (pixels) to read and normalize.
 * @param out_ptr Pointer to a float array where the normalized pixel values 
 *                (0.0, 1.0) will be populated for the classifier.
 * @return 0 indicating a successful memory map, conversion, and read operation.
 */
